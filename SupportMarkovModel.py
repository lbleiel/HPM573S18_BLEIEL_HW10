...
def print_comparative_outcomes(simOutputs_none, simOutputs_anticoag):
    ...
    # difference in number of strokes
    if Settings.PSA_ON:
        number_of_strokes_dif = Stat.DifferenceStatPaired(
        ...
        number_of_strokes_dif = Stat.DifferenceStatIndp(
            name='Decrease in number of strokes',
            x=simOutputs_anticoag.get_if_developed_stroke(),
            y_ref=simOutputs_none.get_if_developed_stroke())
    ...
    # estimate and CI
    estimate_CI = F.format_estimate_interval(
        estimate=number_of_strokes_dif.get_mean(),
        interval=number_of_strokes_dif.get_t_CI(alpha=Settings.ALPHA),
        deci=2)
    print("Average decrease in number of strokes "
          "and {:.{prec}%} confidence interval:".format(1 - Settings.ALPHA, prec=0),
          estimate_CI)
